...
import { Home, Info, Hotel, Utensils, Image, MessageCircle, Phone, Mail, Linkedin, Facebook, Rss, Newspaper } from 'lucide-react';
...
const NavLink = ({ icon, text, sectionId, scrollToSection, isMobile = false, onClick }) => {
    ...
    return (
        <a
            ...
            ${isMobile ? 'text-gray-800 hover:text-blue-600 py-2 w-full justify-center' : 'text-gray-700 hover:text-blue-600'}
            `}
        >
            {icon && <span className="mr-2 group-hover:animate-bounce-icon-small">{icon}</span>}
            <span className="relative z-10">
                {text}
                <span className="absolute bottom-0 left-0 w-full h-0.5 bg-blue-500 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left nav-link-underline-glow"></span>
            </span>
        </a>
    );
};
...
export const SocialMediaIcons = ({ isMobile = false }) => (
    <div className={`flex ${isMobile ? 'flex-col space-y-3' : 'space-x-4'}`}>
        <a href="https://www.facebook.com/share/15ujX1JxHc/?mibextid=wwXIfr" target="_blank" rel="noopener noreferrer" className="text-gray-700 hover:text-blue-600 transition-colors duration-300 transform hover:scale-110 animate-float-subtle-light delay-200" aria-label="Facebook">
            <Facebook size={isMobile ? 24 : 20} />
        </a>
    </div>
);

// Header Component: Dynamic sizing on scroll
const Header = ({ scrollToSection, isMenuOpen, setIsMenuOpen, setShowReviewModal }) => {
    ...
    return (
        <header className={`bg-white/90 backdrop-blur-md shadow-xl py-4 px-6 md:px-12 fixed w-full z-20 transition-all duration-500 transform ${scrolled ? 'py-3 shadow-2xl border-b border-blue-200/50' : 'py-4'} animate-slide-down`}>
            <nav className="container mx-auto flex justify-between items-center relative">
                ...
                <div className="flex items-center md:hidden ml-4">
                    <button
                        ...
                        className="text-gray-700 focus:outline-none p-2 rounded-md hover:bg-gray-200 transition-colors duration-300 transform hover:scale-110 active:scale-90"
                        aria-label="Toggle navigation menu"
                    >
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"> {/* Changed w-8 h-8 to w-6 h-6 */}
            ...
            {/* Mobile Menu Overlay */}
            {isMenuOpen && (
                <div className="md:hidden bg-white absolute top-full left-0 w-full shadow-lg py-4 transition-all duration-300 ease-in-out transform origin-top scale-y-100 z-10">
